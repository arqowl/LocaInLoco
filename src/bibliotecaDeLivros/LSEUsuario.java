...
// Métodos públicos
    ...
    public void inserirUsuario(Usuario al) {
        LSENodeUsuario novo;
        // Verifica, usando a função "buscar" se o Usuario já é cadastrado
        LSENodeUsuario result = this.buscar(al);
        if (result != null) {
            System.out.println("Usuario já existe na lista!");
        } else {
            // Cria e preenche um novo nó com o Usuario que foi passado como parâmetro
            novo = new LSENodeUsuario(al);
            // Insere um novo Usuario no início da lista de Usuarios
            if (this.isEmpty() == true) {
                this.primeiro = novo;
            } else {
